<template>
	<div id="cardList">
		...
		<router-link to='/card'><div class="addCard">添加车牌</div></router-link>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			changeCard(index){
				...
				this.$http({
					method: 'post',
					url: this.httpUrl+'brand/updatepriority',
					data: Qs.stringify({
...
<style scoped="scoped">
	.notes{text-align: center;padding: 0.2rem 0;color: #787878;}
	li{display: flex;justify-content: space-between;background-color: white;border-bottom: 1px solid #CCCCCC;position: relative;overflow: hidden;}
	li:nth-last-of-type(1){border: none;}
	li span:nth-of-type(1){font-size: 0.32rem;width: 80%;padding: 0.3rem 0rem 0.3rem 0.4rem;}
	ul div{width: 100%;text-align: center;padding: 1rem 0;}
	ul div img{width: 30%;}
	.card{font-size: 0.24rem;}
	.parameter{background-color: #F68B1B;color: #FFFFFF;font-size: 0.24rem;display: block;width: 0.96rem;height: 0.34rem;text-align: center;line-height: 0.34rem;border-radius:4px ;margin: 0.3rem 0.5rem 0.3rem 0rem;}
	.addCard{font-size: 0.32rem; width: 80%;height: 0.88rem;background-color: #F68B1B;text-align: center;line-height: 0.88rem;color: #fff;margin: 0.4rem auto;border-radius:5px ;}
	.delete{
		font-size: 0.32rem;
		height: 100%;
