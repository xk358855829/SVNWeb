<template>
	<div id="PSTlist">
		...
		<div class="select">
			...
			<span>道闸编号：</span>
			<el-input v-model="search.deviceld" clearable placeholder="请输入道闸编号"></el-input>
			<span>流量卡号：</span>
			<el-input v-model="search.sim" clearable placeholder="请输入流量卡号"></el-input>
			<span>绑定道闸：</span>
			<el-select v-model="search.isBind" placeholder="请选择">
		...
		<el-dialog title="新增道闸设备" :visible.sync="universal.add">
			<div>
				<span class="spanClassT"><span class="aKey">*</span>道闸编号：</span>
				<el-input v-model="add1.deviceld" clearable placeholder="请输入道闸编号" @blur="focus()" @focus="innert"></el-input><span v-show="tishi">设备编号已存在</span>
				<br /><br />
				
				<span class="spanClassT">SIM卡号：</span>
				<el-input v-model="add1.protocol" clearable placeholder="请输入SIM卡号"></el-input><br /><br />
				<span class="spanClassT">协议版本号：</span>
				<el-input v-model="add1.sim" clearable placeholder="请输入协议版本号"></el-input><br /><br />
...
<!-- 
		...
		<el-dialog title="查看" :visible.sync="universal.look">
			<div class="letg">
				<el-row>
					<el-col :span="12"><div class="grid-content">停车场名称：</div>{{look.parking_name}}</el-col>
					<el-col :span="12"><div class="grid-content">免费时长：</div>{{look.free_time}}</el-col>
				</el-row>
				<el-row> 
					<el-col :span="12"><div class="grid-content">停车场类型：</div>{{look.parking_lot_type}}</el-col>
					<el-col :span="12"><div class="grid-content">收费标准：</div>{{look.price}}</el-col>
				</el-row>
					<div class="grid-content longsize">停车场地址：</div><div>{{look.parking_address}}</div>
			 </div>
		</el-dialog>
			...
			<el-table class='oyTable' :data="tableData" style="width: 100%">
				...
				<el-table-column prop="" label="操作" width="">
					<template slot-scope="scope">
						...
						<el-tooltip class="item" effect="dark" content="远程控制" placement="top">
    						<i class="jilu iconfont icon-yuanchengkongzhi" @click="popup(scope.row)" ></i>
						</el-tooltip>
		...
		<el-dialog title="远程控制" :visible.sync="universal.allotenter">
			<div>
				<span class="spanClass">设备编号：</span>
				<el-input :disabled="true" v-model="allotenter.id" clearable placeholder="请输入内容"></el-input>
				<br /><br />
				<span class="spanClass">设备状态：</span>
				<el-select v-model="allotenter.str" placeholder="请选择设备状态">
					<el-option>未选择</el-option>
					<el-option v-for="item in enter" :key="item.id" :label="item.type" :value="item.type"></el-option>
				</el-select><br /><br />
			...
			<div slot="footer" class="dialog-footer">
				...
				<el-button type="success" class="oyButton" @click="changeGroup">启用</el-button>
			</div>
		</el-dialog>
		<div class="footer">
	...
	export default {
		...
		methods: {
			...
			changeGroup(row){
				...
				axios({
					method:'post',
					url:'http://192.168.2.110:3010//wirite/single/openid',
					data:Qs.stringify(data)
				}).then(function(message){
					console.log(message)
					if(message.data.resCode==0){
...
<style scoped="scoped">
	...
	.longsize{width: 15%!important;text-align: right}
	.iconcolor{background: #409EFF}
	.jilu{
		width: 22px;
		height: 22px;
		display: inline-block;
