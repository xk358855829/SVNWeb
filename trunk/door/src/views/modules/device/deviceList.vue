<template>
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-message__mail-template">
      ...
      <el-table v-loading="dataListLoading" :data="deviceList" border ref="table" :row-key="getRowKey" style="width: 100%;">
      ...
      <div class="pagefooter">
        <el-pagination background v-if="paginationShow" :current-page="page" :page-sizes="[10, 20, 50, 100]" :page-size="limit" :total="deviceTotal" layout="total, sizes, prev, pager, next, jumper"
...
export default {
  ...
  computed: {
    ...mapState(['deviceList','deviceTotal']),
  },
  created(){
    let data = {
      pageNum: 1,
  ...
  methods: {
    ...
    //查看绑定用户
    look(id){
      this.$router.push({name: "device-deviceBind",query: {id :id}})
    },
    ...
    deleteHandle(val) {
      ...
      that
        ...
        .then(() => {
          ...
          this.$http.post(this.$apiUrl.baseUrl + "/s_user/delete",JSON.stringify({ userId: row.userId })).then(
            ({ data: res }) => {
              if (res.resCode != 0) {
