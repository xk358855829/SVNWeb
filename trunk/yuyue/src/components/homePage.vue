<template>
	<div class="homePage" @touchstart="_touchStart()" @touchmove="_touchMove()">
		<div class="logo" :style="{backgroundImage: 'url(' + img1 + ')' }">
			<img @click="myup()" id="wd" src="../../static/image/lpz.png" />
			<img src="../../static/image/aa.png" />
			<!-- <p>车位预约系统</p> -->
		</div>
	...
	export default {
		...
		methods: {
			pcli(){
				axios({
					method:'post',
					url:'http://39.104.113.112:8797/login/queryOpenId',
					data:Qs.stringify({
			...
			saoyisao() {
				...
				this.$http({
					method: 'get',
					url: 'http://114.55.168.103:8797/scan/getJSSDKTicket?url=' + window.location.href.split('#')[0]
					 ...
					 wx.ready(function(){
						 wx.scanQRCode({
							...
							success: function(res) {
								...
								axios({
									method:'post',
									url:that.httpUrl+'web/parkingdevList/selectPrakingDevid',
									data:Qs.stringify({
										devid:deviceId
									})
								}).then(res=>{
									...
									if(res.data.resCode=='500'){
										...
										let orc=res.data.result.ParkingLotInfo
										Indicator.close()
										that.$router.push({path:'/orderTwo',
										query:{id:orc.id,
										price:orc.price,name:orc.parkingName,timeout:orc.timeOut}})
									}
								})
							}
						});
