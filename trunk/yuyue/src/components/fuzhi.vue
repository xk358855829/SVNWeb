        ...
        <ul class="inner">
             <li v-for="(item,index) in tablesplo" style="position: relative;" @click="check(item)" :class="{zonp:item.use_status==-1}">
                   <div style="display:flex;">
                       ...
                       <div style="width:70%;height:30%;margin-top:0.2rem;" class="texts">
                           <div style="display:flex;">
                               <div :class="sstts[(item.useWay)]" v-if="item.use_status==1" style="width:1.16rem;height:0.4rem;text-align:center;line-height:0.4rem;border-radius:5px;margin:0.06rem;margin-top:-0.1rem;color:#fff;">{{item.useWay|change}}</div>
                               <div v-if="item.use_status==-1" style="width:1.16rem;height:0.4rem;text-align:center;line-height:0.4rem;border-radius:5px;margin:0.06rem;margin-top:-0.1rem;color:#fff;background:#999;">{{item.useWay|change}}</div>
                               <div v-if="item.use_status==0" style="width:1.16rem;height:0.4rem;text-align:center;line-height:0.4rem;border-radius:5px;margin:0.06rem;margin-top:-0.1rem;color:#fff;background:#999;">{{item.useWay|change}}</div>
                               <div>杭州速利科技电子券</div>
                           </div>
                           <div style="display:flex;justify-content: space-between;line-height:0.4rem;position: relative;">
                               ...
                               <div class="square" v-if="item.use_status==1"></div>
                               <div style="position: absolute;right:0.32rem;margin-top:-0.04rem;" v-if="item.remark&&item.use_status==1">
                                   <img src="../../static/image/rong.png" alt="">
                               </div>
                               <div style="position: absolute;width:1rem;height:1rem;top:-0.56rem;right:0" v-if="item.use_status==-1">
                                   <img src="../../static/image/zg (1).png" alt="" style="width:100%;height:100%;">
                               </div>
                               <div style="position: absolute;width:1rem;height:1rem;top:-0.56rem;right:0" v-if="item.use_status==0">
                                   <img src="../../static/image/zg (2).png" alt="" style="width:100%;height:100%;">
                               </div>
                           </div>
                           <div style="margin-top:0.2rem;" v-if="item.useWay=='2'">抵扣停车费<span>{{item.sub}}</span>元</div>
                           <div style="margin-top:0.2rem;" v-if="item.useWay=='1'">免费时长<span>{{item.freeTime}}分钟</span></div>
                           <div style="margin-top:0.2rem;" v-if="item.useWay=='3'">满<span>{{item.full}}元</span>减<span>{{item.sub}}元</span></div>
                       </div>
                   </div>
                </li>
        </ul>
...
export default {
    ...
    methods:{
        ...
        use(index){
            if(index==0){
                this.tablesplo=this.saleProducts.c
                if(this.tablesplo.length==0){
                    this.imgl=true;
                }
            }else if(index==1){
                this.tablesplo=this.saleProducts.b
                if(this.tablesplo.length==0){
                    this.imgl=true;
                }
            }else{
                this.tablesplo=this.saleProducts.a
                if(this.tablesplo.length==0){
                    this.imgl=true;
                }
            }
    ...
    mounted(){
        let data={
            ...
            // 
        this.tickets(data)
        let a=localStorage.getItem('tab')
