<template>
	<div id="parkingLot">
		...
		<ul v-if="parameter == 1&&parameterTwo == 0">
			<li @click="select(index)" v-for="(lis,index) in list" :key="index">
				<div class="card">
					<!-- :src="lis.imageaddress"  -->
					<img
						src='https://tse1-mm.cn.bing.net/th?id=OIP._tzSIqdu9Ua3aGfnGBrzfAHaEK&w=286&h=160&c=7&o=5&pid=1.7'
						alt="" 
					/>
					<span class="name">{{lis.parkingName}}</span>
				</div>
				<p class="na"><span>{{lis.parkingName}}</span><span class="km"><img src="../../static/image/dtwz.png"/>{{lis.km}} km</span></p>
	...
	export default{
		...
		mounted: function() {
			...
			that.$http({
				...
				Indicator.close();
				console.log(message);
				let a=message.data.result.ParkingLotDirectionList;
				if(a.length){
					that.parameter=1;
		...
		methods:{
			...
			select(index){
				...
				this.$router.push({path:'/orders',query:{
					...
					'name':this.list[index].parkingName,
					'km':this.list[index].km,
					'totalNumber':this.list[index].carportcount,
					'remaining':this.list[index].ontimecount,
					timeout:this.list[index].timeOut
				}})
			},
...
@import '../style/mixin.scss';
	.title{border-bottom: 1px solid $hui; display: flex;justify-content: space-around;font-size: 0.32rem;background-color: #FFFFFF;position: fixed;top: 0;width: 100%;z-index: 9999;}
	...
	#ggtcc li{display: flex;justify-content: space-between; background-color: #FFFFFF;border-bottom: 1px solid $hui;padding: 0.36rem 0;margin: 0 0.3rem;height: 0.82rem;}
