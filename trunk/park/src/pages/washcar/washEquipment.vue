  ...
  <div id="equipmentList">
    ...
    <div class="Content">
      <el-table class="oyTable" :data="tableData" style="width: 100%">
        ...
        <!-- <el-table-column label="绑定摄像头序号">
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="top">
              <p class="TsMaxWidth">{{ scope.row.bindcarid == 0 ? "未绑定" : scope.row.bindcarid }}</p>
              <div slot="reference" class="name-wrapper">
                <div class="overflowHide">{{ scope.row.bindcarid == 0 ? "未绑定" : scope.row.bindcarid }}</div>
              </div>
            </el-popover>
          </template>
        </el-table-column>
        <el-table-column label="绑定摄像头名称">
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="top">
              <p class="TsMaxWidth">{{ scope.row.bindcarname == 0 ? "未绑定" : scope.row.bindcarname}}</p>
              <div slot="reference" class="name-wrapper">
                <div class="overflowHide">{{ scope.row.bindcarname == 0 ? "未绑定" : scope.row.bindcarname }}</div>
              </div>
            </el-popover>
          </template>
        </el-table-column> -->
        ...
        <el-table-column label="洗车机状态">
          <template slot-scope="scope">
            <div slot="reference" class="name-wrapper">
              <el-tag
                color="#f8c541"
                v-if="scope.row.succ==1"
              >应答成功</el-tag>
              <el-tag
                color="#5daf34"
                v-else-if="scope.row.succ==2"
              >启动成功</el-tag>
              <el-tag
                color="#f8c541"
        ...
        <el-table-column label="操作" width="150">
          <template slot-scope="scope">
            ...
            <el-tooltip v-if="scope.row.bindcar.length>0" class="item" effect="dark" content="解绑摄像头" placement="top">
    ...
    <el-dialog title="绑定摄像头" :visible.sync="universal1.bind">
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-tooltip class="item" effect="dark" content="绑定" placement="top">
                <i class="jilu iconfont icon-yuanchengkongzhi" @click="barrierclick(scope.row.deviceId)"></i>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
      ...
      <div slot="footer"
        ...
        <!-- <el-button type="success"
                   class="oyButton"
                   @click="universal1.bind=false">确定</el-button> -->
      </div>
    </el-dialog>
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-tooltip class="item" effect="dark" content="解绑" placement="top">
                <i class="jilu iconfont icon-bangding" @click="nobindclick(scope.row.carid)"></i>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    handleSearchClick() {
      ...
      getwashEquipment(data).then(function(message) {
        ...
        if (message.data.resCode == 0) {
          ...
          for(var i = 0;i < message.data.count;i++){
            let list = {id:message.data.result[i].id,deviceid:message.data.result[i].deviceid};
            that.equipmentlist.push(list);
            let obj = {
              alias:message.data.result[i].alias,
              date:message.data.result[i].date,
              dealer:message.data.result[i].dealer,
              deviceid:message.data.result[i].deviceid,
              id:message.data.result[i].id,
              number:message.data.result[i].number,
              run:message.data.result[i].run,
              sid:message.data.result[i].sid,
              succ:message.data.result[i].succ,
              bindcar:[]
            }
            axios({
                method: 'post',
                url: that.$store.state.wash_test_url + '/parkingCarId/selectByParkingCarEquipment',
                data: Qs.stringify({deviceid:obj.deviceid})
            }).then(res => {
              console.log(res);
              if (res.data.resCode == 0) {
                if(res.data.result.length>0){
                  obj.bindcar = res.data.result;
                }else{
                  obj.bindcar = 0;
                }                                       
              } else {
