<template>
  <div id="PSTlist">
    ...
    <div class="select">
      <span>归属客户：</span>
      <el-select v-model="search.conGroup"
                 placeholder="请选择">
        <el-option label="未选择"
                   value="0"></el-option>
        <el-option v-for="(item,index) in this.$store.state.conGroupList"
                   :key="item.groupId"
                   :label="item.gropuName"
                   :value="item.groupId"></el-option>
      </el-select>
      <span>联系方式：</span>
      <el-input v-model="search.cardId"
                clearable
                placeholder="请输入内容"></el-input>
      <span>客户账号：</span>
      <el-input v-model="search.deviceType"
                clearable
                placeholder="请输入内容"></el-input>
      <span>设备型号：</span>
      <el-select v-model="search.conGroup"
    ...
    <el-dialog title="新增"
      ...
      <div>
        ...
        <span class="spanClassT">
          <span class="aKey">*</span>设备IC卡：</span>
    ...
    <el-dialog title="绑定"
      ...
      <div>
        ...
        <el-input v-model="allot1.startTime"
                  clearable
                  placeholder="请输入内容"
                  :disabled="true"></el-input><br /><br />

        <span class="spanClass">结束时间：</span>
        <el-input v-model="allot1.endTime"
                  clearable
                  placeholder="请输入内容"
                  disabled="true"></el-input><br /><br />
      ...
      <div slot="footer"
        ...
        <el-button type="success"
                   class="oyButton"
                   @click="allotchange(allot1)">绑定</el-button>
      </div>
    </el-dialog>
    <el-dialog title="查看"
    ...
    <el-dialog title="选择"
      ...
      <div slot="footer"
        ...
        <el-button type="success"
                   class="oyButton"
                   @click="IChideclick">取消</el-button>
        <el-button type="success"
                   class="oyButton"
                   @click="ICclick()">确认</el-button>
      </div>
    </el-dialog>
      ...
      <el-table class='oyTable'
        ...
        <el-table-column label="归属客户">
          <template slot-scope="scope">
            <el-popover trigger="hover"
                        placement="top">
              <p class="TsMaxWidth">{{ scope.row.sex }}</p>
              <div slot="reference"
                   class="name-wrapper">
                <div class="overflowHide">{{ scope.row.sex }}</div>
              </div>
            </el-popover>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    addClick () {
      ...
      // 	return false;
      // }
      // data.deviceType=this.add1.level,
    ...
    searchClick () {
      var start = async function () {
        try {
          ...
          axios({
            method: 'post',
            url: 'http://192.168.2.123:7895/users/find',
            data: Qs.stringify(data)
          }).then(function (message) {
            console.log(message.data)
            if (message.data.resCode == 0) {
