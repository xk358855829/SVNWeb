	...
	<div id="PSTlist">
		...
		<el-dialog title="新增" :visible.sync="universal.add">
			<div>
				...
				<el-input v-model="add.deviceId" clearable placeholder="请输入内容"></el-input><br /><br />

				<!-- <span class="spanClass">设备名称：</span> -->
				<span class="spanClassT"><span class="aKey">*</span>设备识别码：</span>
				<el-input v-model="add.deviceName" clearable placeholder="请输入内容"></el-input><br /><br />

				<span class="spanClassT"><span class="aKey">*</span>产品型号：</span>
				<el-input v-model="add.allot" clearable placeholder="请输入内容"></el-input><br /><br />
		...
		<div class="Content">
			<el-table class='oyTable' v-loading="listLoading" :data="tableData" style="width: 100%">
				...
				<el-table-column label="ICCID">
					<template slot-scope="scope">
				        <el-popover trigger="hover" placement="top">
				        	<p  class="TsMaxWidth">{{ scope.row.conCode }}</p>
				        	<div slot="reference" class="name-wrapper">
				        		<div class="overflowHide">{{ scope.row.conCode }}</div>
				        	</div>
				        </el-popover>
			      </template>
				</el-table-column>
				...
				<el-table-column label="设备编号">
					<template slot-scope="scope">
				        <el-popover trigger="hover" placement="top">
				        	<p  class="TsMaxWidth">{{ scope.row.devCode }}</p>
				        	<div slot="reference" class="name-wrapper">
				        		<div class="overflowHide">{{scope.row.devCode}}</div>
				        	</div>
				        </el-popover>
			      </template>
				</el-table-column>
				
				<el-table-column label="识别码匹配日期" width="100">
					<template slot-scope="scope">
				        <el-popover trigger="hover" placement="top">
				        	<p  class="TsMaxWidth">{{ scope.row.devDate }}</p>
				        	<div slot="reference" class="name-wrapper">
				        		<div class="overflowHide">{{ scope.row.devDate }}</div>
				        	</div>
				        </el-popover>
			      </template>
				</el-table-column>
				...
				<el-table-column label="销售标识" width="">
					<template slot-scope="scope">
				        <el-popover trigger="hover" placement="top">
				        	<p  class="TsMaxWidth">正常销售</p>
				        	<div slot="reference" class="name-wrapper">
				        		<div class="overflowHide">正常销售</div>
				        	</div>
				        </el-popover>
			      </template>
				</el-table-column>
				...
				<el-table-column prop="" label="操作" width="">
					<template slot-scope="scope">
						...
						<el-tooltip class="item" effect="dark" content="解除流量卡匹配" placement="top">
    						<i class="icon-menu el-icon-sort" @click="deleteClick(scope.row)"></i>
						</el-tooltip>
						
						<el-tooltip class="item" effect="dark" content="设备二维码" placement="top">
	...
	export default {
		...
		methods: {
			...
			addClick(){
				...
				if(this.add.deviceId  == ''){
					this.$message.error('带*的数据不能为空哦！');
					return false;
				}
				if(this.add.deviceName == ''){
					this.$message.error('带*的数据不能为空哦！');
					return false;
				}
				if(this.add.allot == ''){
				...
				axios({
					method:'post',
					dataType: 'jsonp',
					url:this.$store.state.http+'/deviceCode/addCode',
			...
			deleteClick(row){
				...
				this.$confirm('此操作将解绑IC卡, 是否继续?', '提示', {
					...
					axios({
						method:'post',
						url:this.$store.state.http+'/deviceCode/delete',
						data:Qs.stringify({
							conCode:row.conCode
						})
					}).then(function(message){
			...
			searchClick(){
				...
				axios({
					method:'post',
					url:this.$store.state.http+'/deviceCode/list',
					data:Qs.stringify(data)
				}).then(function(message){
					console.log(message)
					if(message.data.resCode==0){
						...
						for(var i=0;i<message.data.results.length;i++){
							message.data.results[i].devDate=timeToDate(JSON.stringify(message.data.results[i].devDate.time).slice(0,10))
						}
