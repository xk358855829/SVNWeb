<template>
  <div id="faultList">
    ...
    <div class="Content">
      ...
      <div v-if="dishow==5"
           class="order identical">停车场名称：{{paking_lot_name}}</div>
      <div v-if="dishow==1"
           class="order identical">停车场名称：xxxxx</div>
      <div class="identical">
        <p v-if="dishow==5">入场时间：{{startdate}}</p>
        <p v-if="dishow==1">入场时间：xxxxx</p>
        <p v-if="dishow==5"
           class="last">停车时长：{{longdate}}</p>
        <p v-if="dishow==1"
           class="last">停车时长：xxxxx</p>
      </div>
      <div class="identical">
        <p v-if="dishow==5">已付金额：{{paid_price}}</p>
        <p v-if="dishow==1">已付金额：xxxxx</p>
        <p v-if="dishow==5"
           class="last">优惠金额：{{onsale_price}}</p>
        <p v-if="dishow==1"
           class="last">优惠金额：xxxxx</p>
      </div>
      <div class="identical">
        <p v-if="dishow==5">停车费用：{{price}}</p>
        <p v-if="dishow==1">停车费用：xxxxx</p>
        <p v-if="dishow==5"
           class="last">超时时间：{{time_out}}</p>
        <p v-if="dishow==1"
           class="last">超时时间：xxxxx</p>
      </div>
      <div class="identical">
        <p>
          <el-checkbox @change="changeboxticket"></el-checkbox>
          是否使用优惠券</p>
        <p class="last">输入优惠券码：
          <el-input style="width:200px;margin-left:-50px;"
                    v-model="ticket"></el-input>
        </p>
      </div>
      <!-- <p class="total">费用总计：{{price}}元</p> -->
      <div class="pay identical"
           style="margin-top:40px;"
           v-if="changehuan==2"
           @click="changeClick">支付</div>
      <div class="pay identical"
           style="margin-top:40px;"
           v-if="changehuan==1">支付</div>
    </div>
