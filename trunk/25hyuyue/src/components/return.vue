  ...
  <div class="body">
    <div id="index-center">
      ...
      <div class="index-center-center">
        <img class="index-switch"
             src="../../static/image/swith_3.png" />
      </div>
    ...
    <div style="font-size:15px;text-align:center;width:80%;height:0.8rem;background:#fb851c;margin:0.5rem auto;border-radius:5px;line-height:0.8rem;color:#fff;"
         @click="go">升起</div>
  </div>
...
import { serchstates, showTime } from './api/api.js'
...
export default {
  ...
  beforeRouteLeave (to, from, next) {
    if (to.name == 'pay' && from.name == 'return') {
      next({ path: '/homePage' })
    } else if (to.name == 'payyuyue' && from.name == 'return') {
      next({ path: '/homePage' })
    } else {
      next()
    }
        ...
        if (action == 'confirm') { //确认的回调
          ...
          if (this.statePH == 0) {
            ...
            let data = {
              ...
              resId: localStorage.getItem('setOrder'),
