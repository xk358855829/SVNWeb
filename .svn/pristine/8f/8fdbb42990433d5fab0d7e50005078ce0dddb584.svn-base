<template>
		<el-dialog :visible.sync="visible" :title="!dataForm.userid ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false">
				<el-form :model="dataForm" :rules="rules" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()" label-width="120px">
						...
						<el-row v-if="(dataForm.userid&&dataForm.type!=0)||dataForm.userid==''">
								<el-col :span="12">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 获取信息
            getInfo() {
                this.$http.post(this.$apiUrl.baseUrl + '/webUser/findByPrimaryKey',JSON.stringify({userid:this.dataForm.userid})).then(res => {                    
                    if (res.code !== 0) {
            ...
            dataFormSubmitHandle: debounce(
                function () {
                    this.$refs["dataForm"].validate(valid => {
                        ...
                        console.log(data);
                        this.$http.post(this.$apiUrl.baseUrl + url, JSON.stringify(data)).then(res => {
                            if (res.code !== 0) {
                                return this.$message.error(res.message);
                            }
                            this.$message({
