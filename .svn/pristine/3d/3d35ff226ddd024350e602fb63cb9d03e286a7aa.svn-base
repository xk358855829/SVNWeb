<template>
	<div id="offReport">
		...
		<div class="select">
			<span>归属客户：</span>
			<el-select v-model="search.tercept" placeholder="请选择">
				<el-option>未选择</el-option>
				<el-option v-for="item in affiliation" :key="item.id" :label="item.type" :value="item.type"></el-option>
			</el-select>
			<span>设备编号：</span>
			<el-input v-model="search.id" clearable placeholder="请输入内容"></el-input>
			<span>在线离线：</span>
			<el-select v-model="search.onoffline" placeholder="请选择">
				<el-option>未选择</el-option>
		...
		<div class="Content">
			<el-table class="oyTable" :data="tableData" style="width: 100%">
				...
				<el-table-column label="归属客户" width="120">
					<template slot-scope="scope">
				        <el-popover trigger="hover" placement="top">
				...
				<el-table-column prop="" label="操作" width='150'>
					<template slot-scope="scope">
						<el-tooltip class="item" effect="dark" content="历史记录" placement="top">
    						<i class="icon-change el-icon-time" @click="changeShow(scope.row)" ></i>
						</el-tooltip>


						
						<el-tooltip  class="item" effect="dark" content="地图定位" placement="top">
    						<i class="icon-delete el-icon-location" @click="deleteClick(scope.row)"></i>
						</el-tooltip>
					</template>
				</el-table-column>
			</el-table>
