  ...
  <div id="faultList">
    ...
    <div class="select">
      ...
      <el-select v-model="search.order_status"
        ...
        <el-option v-for="item in online1"
                   :key="item.id"
                   :label="item.type"
    ...
    <div class="select">
      <span>开始时间：</span>
      <el-date-picker v-model="search.start_pay_time"
                      type="date"
                      placeholder="选择日期"
                      value-format="yyyy-MM-dd"></el-date-picker>
      <span>结束时间：</span>
      <el-date-picker v-model="search.end_pay_time"
                      type="date"
                      placeholder="选择日期"
                      value-format="yyyy-MM-dd"></el-date-picker>
      <span>&nbsp;&nbsp;&nbsp;支付类型：</span>
      <el-select v-model="search.pay_type"
                 placeholder="请选择">
        <el-option>未选择</el-option>
          ...
          <template slot-scope="scope">
            <el-popover>
              ...
              <div slot="reference"
                ...
                <div>
                  <img v-if="scope.row.pay_type==5"
                       src="../../../static/image/cash.png"
                       alt="">
                  <img v-if="scope.row.pay_type==2"
                       src="../../../static/image/alipay.png"
                       alt="">
                  <img v-if="scope.row.pay_type==3"
                       src="../../../static/image/wechat.png"
                       alt="">
                  <img v-if="scope.row.pay_type==1"
                       src="../../../static/image/noninductive.png"
                       alt="">
                  <img v-if="scope.row.pay_type==6"
                       src="../../../static/image/teq.png"
                       alt="">
                  <img v-if="scope.row.pay_type==0"
        ...
        <el-table-column label="订单状态"
          ...
          <template slot-scope="scope">
            ...
            <div slot="reference"
              ...
              <el-tag color="#5daf34"
                      v-if="scope.row.order_status=='完成'">已完成</el-tag>
              <el-tag color="#f8c541"
                      v-if="scope.row.order_status=='已退款'">{{ scope.row.order_status }}</el-tag>
              <el-tag color="#c9521e"
                      v-if="scope.row.order_status=='未支付'">{{ (scope.row.order_status) }}</el-tag>
            </div>
            <!-- </el-popover> -->
          </template>
        </el-table-column>
    ...
    <el-dialog title="查看"
      ...
      <div class="letg">
        <el-row>
          <el-col :span="12">
            <div class="grid-content">停车场名称：</div>{{look.parking_lot_name}}</el-col>
          <el-col :span="12">
            <div class="grid-content">支付类型：</div>{{look.pay_type}}</el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <div class="grid-content">车牌号码：</div>{{look.carnumber}}</el-col>
          <el-col :span="12">
            <div class="grid-content">商户订单：</div>{{look.out_trade_no}}</el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <div class="grid-content">订单状态：</div>{{look.order_status}}</el-col>
          <el-col :span="12">
            <div class="grid-content">收单时间：</div>{{look.date}}</el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <div class="grid-content">停车时长：</div>{{look.longdate}}</el-col>
          <el-col :span="12">
            <div class="grid-content">优惠金额：</div>{{look.onsale_price}}</el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <div class="grid-content">入场时间：</div>{{look.startdate}}</el-col>
          <el-col :span="12">
            <div class="grid-content">付款金额：</div>{{look.price}}</el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <div class="grid-content">支付时间：</div>{{look.pay_time}}</el-col>
          <el-col :span="12">
            <div class="grid-content">已付金额：</div>{{look.paid_price}}</el-col>
        </el-row>
      </div>
    ...
    <el-dialog title="退款"
      ...
      <el-row>
        ...
        <el-col :span="12">
          ...
          <el-input :disabled="true"
                    v-model="change.pay_time"
                    clearable
                    placeholder="请输入内容"></el-input><br /><br />
          <span class="spanClass">退款金额：</span>
          <el-input :disabled="false"
                    v-model="change.tmoney"
                    clearable
...
export default {
  ...
  computed: {
    ...mapState(['online1', 'apliyparkName']),
    ...mapGetters({
  ...
  methods: {
    ...
    export2Excel () {
      ...
      axios({
        method: 'post',
        url: that.$store.state.url + '/feeOrder/orders',
        ...
        if (message.data.resCode == "0") {
          ...
          require.ensure([], () => {
            ...
            const filterVal = ['carnumber', 'parking_id', 'price', 'pay_type', 'order_status', 'longdate', 'pay_time'];
    ...
    deleteClick (row) {
      ...
      this.$confirm('此操作将删除该条数据, 是否继续?', '提示', {
        ...
        axios({
          method: 'post',
          url: 'http://39.104.113.112:2088/feeOrder/delOrderByOutTradeNo',
          data: Qs.stringify({
    ...
    tmoenypay (row) {
      ...
      this.change.trade_no = row.trade_no
      if (this.change.trade_no == '') {
        this.change.trade_no = '无'
      }
    ...
    paychange () {
      ...
      let data = {}
      if (this.change.trade_no == '' || this.change.pay_type == 2 || this.change.pay_type == 1) {
    ...
    searchClick () {
      ...
      if (this.search.car_number) {
        data.carnumber = this.search.car_number
      }
      // consoe.log(this.search.order_status)
      if (this.search.order_status) {
        data.order_status = this.search.order_status
      }
      ...
      let that = this;
      that.tableData = [];
      searchClicks('/feeOrder/orders', data).then(message => {
        if (message.data.resCode == 0) {
          for (var i = 0; i < message.data.result.length; i++) {
            if (message.data.result[i].order_status == 0) {
              message.data.result[i].order_status = '完成'
            ...
            if (message.data.result[i].longdate == '') {
              ...
              message.data.result[i].longdate = message.data.result[i].longdate
            }
