      ...
      <el-table v-loading="dataListLoading" :data="dataList" border ref="table" style="width: 100%;">
        ...
        <el-table-column :label="$t('handle')" fixed="right" align="center" width="80">
          <template slot-scope="scope">
            <!-- <el-button type="text" size="small" @click="unbound(scope.row)">解绑</el-button> -->
            <el-tooltip class="item" effect="dark" content="解绑" placement="top">
              <i class="icon-See el-icon-link" @click="unbound(scope.row)"></i>
            </el-tooltip>
...
import { deviceBind, toUnbound } from "../../../api/api";
...
export default {
  ...
  created(){
    ...
    if(this.$route.query.id){
      this.dataForm.deviceId = this.$route.query.id;
    }    
  },
  mounted() {
    this.getDataList();
  ...
  methods: {
    ...
    unbound(val){
      ...
      that
        .$confirm("此操作将解除绑定, 是否继续?", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        })
        .then(() => {
          toUnbound(data).then(res => {
            console.log(res);
