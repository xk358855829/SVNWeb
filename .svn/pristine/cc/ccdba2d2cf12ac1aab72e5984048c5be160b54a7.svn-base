          ...
          <div style="width:70%;height:30%;margin-top:0.2rem;">
            <div style="display:flex;">
              <div :class="sstts[(item.useWay)]"
                   style="width:0.58rem;height:0.2rem;text-align:center;line-height:0.23rem;border-radius:5px;margin:0.03rem;margin-top:-0.05rem;color:#fff;">{{item.useWay|change}}</div>
              <div>杭州速利科技电子券</div>
            </div>
            <div style="display:flex;justify-content: space-between;line-height:0.3rem">
              ...
              <div :class="sstt[(item.useWay)]"
                   style="margin-right:8px;width:0.58rem;height:0.2rem;border-radius:10px;text-align:center;line-height:0.2rem;"
                   @click="shiyong(item)">立即使用</div>
            </div>
            <div style="line-height:0.3rem"
                 v-if="item.useWay=='2'">抵扣停车费
