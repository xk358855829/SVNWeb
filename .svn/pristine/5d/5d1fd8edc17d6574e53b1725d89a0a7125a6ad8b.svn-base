  ...
  <div id="pageHome">
    <el-container class='oyContainer'>
      <el-header class='oyHeader'>
        <span class="icommy"><img src='../../static/qee.png' /></span>
        <span class="siaze">智慧泊车设备服务云平台</span>
        <div class="imgset">
          <!-- <router-link tag="div" to='' class="homeTop">
					<img src="../../static/image/powr2.png" alt="" class="imglink">
          ...
          <el-dropdown trigger="click"
            ...
            <span class="el-dropdown-link">
              <img src="../../static/image/people.png"
                   class="imglink"
                   alt="">
              <i class="titles">{{userAlias}}</i>
            </span>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item>
                <span @click='personalClick'>个人中心</span>
              </el-dropdown-item>
              <el-dropdown-item>
        ...
        <el-dialog title="个人中心"
          ...
          <div class="oYpersonal">
            <span class="oYspan"
                  :class="sty==1?'styles':''"
                  @click="sty = 1">查看详情</span>
            <span class="oYspan"
                  :class="sty==2?'styles':''"
                  @click="sty = 2">修改资料</span>
            <span class="oYspan"
                  :class="sty==3?'styles':''"
                  @click="sty = 3">修改密码</span>
          </div>
          <div v-if="sty == 1">
            <span class="oYperSty">账号：</span>
            <span>{{data.userName}}</span><br /><br />
            <span class="oYperSty">用户类型：</span>
            <span>{{data.type}}</span><br /><br />
            <span class="oYperSty">用户名：</span>
            <span>{{data.userAlias}}</span><br /><br />
            <span class="oYperSty">绑定手机：</span>
            <span>{{data.phoneNum}}</span><br /><br />
            <span class="oYperSty">绑定邮箱：</span>
          ...
          <div slot="footer"
            ...
            <el-button type="success"
                       class="oyButton"
                       @click="personal = false">取消</el-button>
            <el-button v-if="sty == 2"
                       type="success"
                       class="oyButton"
                       @click="changeData">保存</el-button>
            <el-button v-if="sty == 3"
                       type="success"
                       class="oyButton"
                       @click="changePassWord">保存</el-button>
          </div>
        </el-dialog>
      ...
      <el-container>
        <el-aside :class="{lastadd:yes,firstadd:no}">
          <el-radio-group v-model="isCollapse">
            <span class="guide-nav-angle"
                  @click="slidehide">
              <img class="guide-nav-angle-icon"
                   src="../../static/image/nav.png"
                   alt="">
            </span>
          ...
          <el-menu default-active="2"
            ...
            <el-menu-item index="1">
              <div class="el-submenu__title"
                   @click="zhuan"
                   style="padding: 0 0">
                <i><img src="../../static/homep.png"
                       alt=""></i>
                <span class="oYtitle"
                      slot="title">首页总览</span>
              </div>
            </el-menu-item>
            <el-submenu index="2"
                        v-for='(activeA,index) in this.$store.state.activeName'
                        :index="index+''"
                        :key='index'>
              <template slot="title">
                <i><img :src="'./static/image/icon/'+activeA.classA.menuAddress"
                       alt=""></i>
                <span class="oYtitle"
                      slot="title">{{activeA.classA.menuName}}</span>
              </template>
              <el-menu-item-group class='oYmenuItemGroup'>
                <el-menu-item class='oYelMenuItem'
                              v-for="(activeB,indey) in activeA.classB"
                              :index="index+'-'+indey"
                              @click='toClick(activeB.menuPagename)'
                              :key='indey'>
                  <div><img :src="'./static/image/icon/'+activeB.menuAddress" /></div>
                  <div class="oYname"
                       v-if="timehide">{{activeB.menuName}}</div>
                </el-menu-item>
              </el-menu-item-group>
            </el-submenu>
          </el-menu>
...
export default {
  ...
  methods: {
    ...mapActions([
      'parktitle', 'typepays', 'facility', 'getSalePlans', 'getParkingLotList'
    ]),
    slidehide () {
      this.yes = !this.yes;
      this.no = !this.no;
    ...
    toClick (val) {
      ...
      this.$router.push({ path: '/pageHome/' + val, query: { id: val } })
    },
    linkOut () {
...
.oYelMenuItem > div:nth-of-type(1) {
  width: 36px;
  color: #fff;
}
.oyInp {
...
.oyButton {
  ...
  padding: 0 !important;
  border: 1px solid #a2cc6d !important;
  background: #a2cc6d !important;
...
.oyTable th:nth-of-type(1) {
  border-bottom-left-radius: 10px;
  border-top-left-radius: 10px;
}
.oyTable th:nth-last-of-type(2) {
...
.oyTableR th:nth-of-type(1) {
  border-bottom-left-radius: 0px;
  border-top-left-radius: 0px;
}
.oyTableR th:nth-last-of-type(2) {
  border-bottom-right-radius: 10px;
  border-top-right-radius: 10px;
}
...
.el-message-box__btns > .el-button {
  background-color: #a2cc6d !important;
  border-color: #a2cc6d !important;
  color: #ffffff !important;
}
...
.oyCollapseItem .el-collapse-item__header:hover {
...
.contentContent > .el-row {
  height: 45px;
  text-align: center;
  font-size: 14px;
}
.contentContent > .el-row:nth-of-type(even) {
  background-color: #f2f6fa;
}
.contentContent > .el-row:hover {
  background-color: #e3eaf3;
}
.contentContent > .el-row > div {
  line-height: 45px;
}
header {
  /* padding-bottom: 30px; */
