...
">
          <input type="text"
                 ...
                 class="num"
                 readonly="readonly"
                 style="text-align: center;border: none;outline: none;background: #fff; width:100%;font-size:0.15rem">
        </div>
      ...
      <p style="padding-bottom:0.18rem;">
        <span class="comp">温馨提示：有效期</span>
        <span class="gouqi"
              style="font-size: 12px;color:rgba(255,59,59) ">{{startTime}}</span>
        <span class="comp">-</span>
        <span class="gouqi"
              style="font-size: 12px;color:rgba(255,59,59) ">{{endTime}}</span>
      </p>
    </footer>
    <div class="paytootsl"
         @click="dunh"
...
export default {
  ...
  mounted () {
    ...
    that.$http({
      method: 'post',
      url: `${that.httpUrl}/MarkRecords/findMarkForOneByParkingId`,
      data: Qs.stringify({
        tel: localStorage.getItem('tel'),
        parkingId: localStorage.getItem('parkingIds') ? localStorage.getItem('parkingIds') : localStorage.getItem('parkingId')
      })
    }).then(res => {
      Indicator.close()
      that.pagefile = true
