<template>
  <div style="width:100%;background:#efefef;overflow: scroll;"
       class="rating_page">
...
　　　　ref="loadmore"> -->
    ...
    <ul class="recordli"
      ...
      <li v-for="item in uncompleteList"
        ...
        <h5 style="justify-content: space-between;">
          <span style="font-size:0.13rem;    color: #333333;">时间：{{item.start_time}}-{{item.end_time}}</span>
        ...
        <div class="la"
          ...
          <div>
            <div style="margin-top:0.1rem;margin-left:0.25rem;">
              <i class="s"
                 style="margin-right:0.1rem;"></i>车主姓名: {{item.name}}
              <span style="float:right;margin-right:0.2rem">充值类型：{{item.type}}</span>
            </div>
            <div style="margin-top:0.1rem;margin-left:0.25rem;">
              <i class="s"
                 style="margin-right:0.1rem;"></i>车牌号码: {{item.car_number}}
              <span style="float:right;margin-right:0.2rem">充值金额：{{item.money}}&nbsp;元</span>
            </div>
            <div style="margin-top:0.1rem;margin-left:0.25rem;">
              <i class="e"
                 style="margin-right:0.1rem;"></i>停车场名称: {{item.parking_name}}
    ...
    <div v-if="imgal"
         style="padding-top:1rem;z-index:22;">
...
import { deloutrec } from './api/api.js'
export default {
  name: 'record',
  data () {
    return {
  ...
  methods: {
    ...
    loadMore1 () {
      ...
      this.$http({
        method: 'post',
        url: this.httpUrl + '/openVIPRecords/selectVIPRecords',
        data: Qs.stringify({
          page: 1,
          pageSize: 100,
          userId: 'SURIOT',
          dealer: 123,
          // car_number: a[0].brandname,
