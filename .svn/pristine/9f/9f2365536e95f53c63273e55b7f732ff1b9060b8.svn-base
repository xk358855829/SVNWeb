    ...
    <div class="select">
      ...
      <!-- <span>停车场名称：</span>
			<el-select v-model="search.parkingName" placeholder="请选择">
				...
				<el-option v-for="item in apliyparkName" :key="item.parking_id" :label="item.type" :value="item.parking_name"></el-option>
			</el-select> -->
      <span>订单流水号：</span>
      <el-input v-model="search.out_trade_no"
                ...
                placeholder="请输入内容"></el-input>
      <!-- <span>退款流水号：</span>
			<el-input v-model="search.out_refund_no" clearable placeholder="请输入内容"></el-input> -->
    </div>
    <div class="select">
...
import { mapActions, mapState } from 'axios'
export default {
  name: 'equipmentList',
  data () {
    return {
  ...
  methods: {
    ...
    searchClick () {
      ...
      if (this.search.out_refund_no) {
        data.out_refund_no = this.search.out_refund_no
      }
      if (this.search.out_trade_no) {
        data.out_trade_no = this.search.out_trade_no
      }
